...
function renderProducts(list = products) {
  grid.innerHTML = list.map(product => `<article class="product-card"><div class="product-image"><span class="product-visual" role="img" aria-label="${product.name}">${product.visual}</span>${product.discount ? `<span class="discount">${product.discount} تخفیف</span>` : ''}</div><div class="product-info"><span class="product-category">${product.category}</span><h3>${product.name}</h3><div class="price-row"><div>${product.oldPrice ? `<span class="old-price">${money(product.oldPrice)}</span>` : ''}<span class="price">${money(product.price)}</span></div><button class="add-product" type="button" data-id="${product.id}" aria-label="افزودن ${product.name} به سبد خرید">+</button></div></div></article>`).join('');
...
function renderCart() { const box = document.querySelector('.cart-items'); const count = cart.reduce((sum,item) => sum + item.quantity, 0); const total = cart.reduce((sum,item) => { const product = products.find(p => p.id === item.id); return sum + product.price * item.quantity; }, 0); document.querySelector('.cart-count').textContent = faDigits(count); document.querySelector('.cart-total').textContent = money(total); box.innerHTML = cart.length ? cart.map(item => { const product = products.find(p => p.id === item.id); return `<div class="cart-item"><div class="cart-item-image">${product.visual}</div><div class="cart-item-info"><strong>${product.name}</strong><small>${money(product.price)}</small><div class="quantity"><button type="button" data-action="decrease" data-id="${product.id}" aria-label="کاهش تعداد">−</button><span>${faDigits(item.quantity)}</span><button type="button" data-action="increase" data-id="${product.id}" aria-label="افزایش تعداد">+</button></div></div><button class="remove-item" type="button" data-action="remove" data-id="${product.id}" aria-label="حذف ${product.name}">×</button></div>`; }).join('') : '<div class="empty-cart"><p>سبد خرید شما خالی است.</p><p>محصولات مورد علاقه‌تان را به سبد اضافه کنید.</p></div>'; }
...
function filterProducts(category = 'همه') { const term = searchInput.value.trim().toLowerCase(); const list = products.filter(product => (category === 'همه' || product.category === category) && product.name.toLowerCase().includes(term)); renderProducts(list); }
...
searchInput.addEventListener('input', () => { const active = document.querySelector('.filter.active'); filterProducts(active ? active.dataset.filter : 'همه'); });
